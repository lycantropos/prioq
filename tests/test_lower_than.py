...
def test_irreflexivity(priority_queue: PriorityQueue) -> None:
    assert not priority_queue < priority_queue


@given(strategies.priority_queues_pairs)
def test_asymmetry(priority_queues_pair: PriorityQueuesPair) -> None:
    first_queue, second_queue = priority_queues_pair

    assert implication(first_queue < second_queue,
                       not second_queue < first_queue)
...
@given(strategies.priority_queues_pairs)
def test_connection_with_greater_than(priority_queues_pair: PriorityQueuesPair
                                      ) -> None:
    first_queue, second_queue = priority_queues_pair

    assert equivalence(first_queue < second_queue, second_queue > first_queue)
